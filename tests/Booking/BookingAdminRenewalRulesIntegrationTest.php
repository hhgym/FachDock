...
final class BookingAdminRenewalRulesIntegrationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->databaseName = $prefix . '_admin_renewal_' . bin2hex(random_bytes(4));
    ...
    public function testAdminPreviewAndRenewalForceDifferentLockerForGradeSeven(): void
    {
        ...
        $targets = (new BookingLifecycleAdminService($this->pdo(), $evaluator))->renewalTargets(1);

        self::assertCount(1, $targets);
        self::assertSame(7, $targets[0]['projected_grade']);
        self::assertTrue($targets[0]['requires_change']);
        self::assertSame(2, $targets[0]['locker_id']);
        self::assertSame('B-01-1', $targets[0]['locker_short_name']);

        $newBookingId = (new BookingLifecycleService($this->pdo(), $evaluator, 14))->renew(
            $this->staff(),
            1,
            2,
            false,
        ...
        self::assertSame(
            2,
            (int) $this->pdo()->query('SELECT locker_id FROM locker_occupancies WHERE booking_id = ' . $newBookingId)->fetchColumn(),
        );
        $event = $this->row(
            "SELECT old_locker_id, new_locker_id, actor_type FROM booking_lifecycle_events WHERE booking_id = 1 AND event_type = 'renewed'"
        );
        self::assertSame(1, (int) $event['old_locker_id']);
        self::assertSame(2, (int) $event['new_locker_id']);
        self::assertSame('staff', $event['actor_type']);
    }

    public function testAdminRenewalUsesAlternativeWhenCurrentLockerIsOccupiedInTargetYear(): void
    {
        ...
        $this->pdo()->exec(
            "INSERT INTO students (id, matrikelnummer, first_name, last_name, class_name, grade, active, created_at, updated_at) VALUES (2, '1002', 'Ada', 'Andere', '9-1', 9, 1, NOW(), NOW())"
        );
        $this->pdo()->exec(
            "INSERT INTO bookings (id, student_id, school_year_id, status, projected_grade, valid_from, valid_until, initiated_by_type, initiated_by_id, annual_fee_cents, charged_fee_cents, proration_months, student_snapshot, rule_snapshot, created_at, updated_at) VALUES (2, 2, 2, 'active', 9, '2027-08-01', '2028-07-31', 'staff', NULL, 3000, 3000, 12, '{}', '{}', NOW(), NOW())"
        );
    ...
    {
        return new AuthenticatedStaff(99, 'admin', 'Admin', 'admin@example.test', StaffRole::Administrator, 1);
    }
